...
## 9. 重构定稿（2026-09-16 下午）：obsidian-mimic，配方-槽位模型

用户复盘后定名 **obsidian-mimic**（目录同步改名），并把"拟态矛盾组"
抽象回 mimic 本意——**模仿-扭曲**：

- **配方（Recipe）= 舞台(worldview) + 禁则(forbidden) + 提示词槽位(slots)**。
  槽位三种控件（select/number/text），每个槽位自带 prompt 模板
  （`{value}` 为用户值，`{label}` 为显示名）；生成时按槽位顺序拼进
  【模仿-扭曲参数】区。
- "立场A/立场B"不再是代码概念——只是用户用两个 text 槽位 + 倾斜 select
  自行组装的一种结构（示范配方"产业博弈"即如此组装）。
- system prompt 改模仿-扭曲框架：模仿底线（核心定义原样或等价）+
  扭曲边界（不编造、遵禁则）。
- **内置 2 个示范配方**（初始配置）：产业博弈（立场对结构示范）、
  新闻滤镜（轻量自由示范）；另有"导入示范配方"命令（追加不覆盖）。
- 设置页附**使用示例**（"课堂讲授的保守教授"配方组装法）。
- migrate 不再迁矛盾组（配方内置 + GUI 演化）；产物 frontmatter 记
  `recipe` + `params`（槽位快照）；输出目录默认改为 `拟态/`。
